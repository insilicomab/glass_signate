...
for train_index, val_index in skf.split(X_train, Y_train):
    ...
    metrics=['accuracy'], # can be `metrics=['RootMeanSquaredError']` for regression task
    ...
    model, history = dt.fit(x_train, y_train, epochs=1000)
    sc = dt.evaluate(x_valid, y_valid, batch_size=512, verbose=0)
